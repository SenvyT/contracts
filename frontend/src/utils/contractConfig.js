...
class ContractConfig {
  ...
  async getContractAddress() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Contract address loaded from deployment:', this.contractAddress);
        return this.contractAddress;
      }
    } catch (error) {
      console.warn('Could not load from ignition deployment:', error);
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (artifact.networks && artifact.networks[networkId]) {
          ...
          console.log('Contract address loaded from artifacts:', this.contractAddress);
          return this.contractAddress;
        }
      }
    } catch (error) {
      console.warn('Could not load from artifacts networks:', error);
    }

    // Final fallback: use default
    this.contractAddress = '0x5FbDB2315678afecb367f032d93F642f64180aa3';
    console.log('Using fallback contract address:', this.contractAddress);
    return this.contractAddress;
  }
  ...
  async getContractABI() {
    ...
    try {
      ...
      console.warn('Could not load ABI from artifacts:', error);
      throw new Error('Failed to load contract ABI. Please ensure artifacts are copied to public folder.');
    }
  }
